...
sys.path.append('..')
from src.meant.meant import meant, languageEncoder, visionEncoder
...
class customDataset_stocknet(Dataset):
    ...
    def __len__(self):
        return len(self.labels)
    def __getitem__(self, idx):
        return {'input_ids':self.tweets[idx], 'prices':self.prices[idx], 'attention_mask':self.attention_masks[idx], 'label':self.labels[idx]}
