...
# for CRF layer for the tweet7 task
       ...
       # train_f1_scores = []
        ...
        for ep in range(self.num_epochs):
            ...
            for train_index in range(0, len(self.X_train) - self.train_batch_size, self.train_batch_size):
                ...
                # processing the mask on device takes up too much space
            ...
            # for early stopping, we want to use validation loss
...
if __name__=='__main__':
    ...
    parser.add_argument('-lrst', '--learning_rate_scheduler_type', type=str, help='The type of learning rate scheduler to use.', default='linear_warmup')
    parser.add_argument('-hfm', '--hugging_face_model', type=str, help='If we want to finetune/pretrain a model from Hugging face.', default= None)
    ...
    parser.add_argument('-crf', '--crf', type=bool, help='Conditional Random Field?', default=False)
    parser.add_argument('-lrwp', '--lr_warmup_step_ratio', type=float, help="Warmup ratio for our learning rate scheduler", default=0.0)
    ...
    if(args.epoch == 0):
        if args.hugging_face_model is not None:
            if args.pretrained is True:
                ...
                # define this in the training script itself
                # implement this shit
                if(args.crf):
                    raise NotImplementedError('Conditional random fields not implemented')
                else:
    ...
    if(args.hugging_face_tokenizer is not None):
        tokenizer = AutoTokenizer.from_pretrained(args.hugging_face_tokenizer, add_prefix_space=True)

    # Generalized training loop will allow for custom datasets
    if(args.hugging_face_data is None):
        ...
        if(args.hugging_face_data == 'adsabs/WIESP2022-NER'):
            ...
            dataset_name = 'jnlpba'
        elif(args.hugging_face_data == 'tner/tweetner7'):
            dataset_name = 'tweet7'
        else:
            dataset_name = args.hugging_face_data

    total_step = len(train) * args.num_epochs
    num_warmup_steps = int(total_step * args.lr_warmup_step_ratio)
    # load incrementally
    ...
    elif(args.learning_rate_scheduler_type == 'linear_warmup'):
        lr_scheduler = transformers.get_linear_schedule_with_warmup(optimizer, 
            num_warmup_steps=num_warmup_steps, num_training_steps=total_step)
    else:
