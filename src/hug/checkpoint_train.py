...
#                to_tokenize = self.join_examples(self.train_data[train_index:train_index+(self.train_batch_size * self.join_size)])
        ...
        if(self.epoch > 0):
            ...
            if(abs(val_f1 - prev_f1) < self.stoppage):
                print('Cancellation')
                # the epoch number we want to propogate onto the test execution
                print('Epoch number: ', self.epoch + 1)
            else:
                print('Continuation')
...
if __name__=='__main__':
    ...
    parser.add_argument('-s', '--stoppage', type=float, help='Stoppage value', default=1e-3)
    ...
    # Define the word you want to search for
        ...
        if search_word in file_content:
            ...
            epoch_number = int(epoch_number_str) 
            print('Cancellation')
            # the epoch number we want to propogate on
            print('Epoch number: ', epoch_number)
            # stoppage has been reached
            sys.exit()
