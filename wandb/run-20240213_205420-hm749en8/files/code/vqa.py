...
from utils import f1_metrics, vqa_dataset, FilteredDataset
from torch.utils.tensorboard import SummaryWriter
import wandb
import pandas as pd
...
if __name__=='__main__':
    ...
    train_data = load_dataset("Graphcore/vqa", split="train[0:10000]").remove_columns(['question_type', 'answer_type', 'question_id'])
    ...
    val_data = load_dataset("Graphcore/vqa", split="validation[0:1000]").remove_columns(['question_type', 'answer_type', 'question_id'])
    ...
    test_data = load_dataset("Graphcore/vqa", split="validation[1000:3000]").remove_columns(['question_type', 'answer_type', 'question_id'])
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.train_batch_size, shuffle=False, pin_memory=True) 
        ...
        if args.hugging_face_model is True:
            if args.pretrained is True:
                ...
                if args.model_name == 'vl_bert':
                    ...
                    model.classifier = nn.Sequential(nn.Linear(768, 1536, bias=True), nn.LayerNorm(1536), nn.GELU(), nn.Linear(1536, num_classes))
